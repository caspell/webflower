...
class webflower_Editor {

	private $contact_form;
	private $panels = array();

	public function __construct( WebFlower_Form $contact_form ) {
		$this->contact_form = $contact_form;
	}

	public function add_panel( $id, $title, $callback ) {
		if ( webflower_is_name( $id ) ) {
...
function webflower_editor_panel_form( $post ) {
?>
<h2><?php echo esc_html( __( 'Form', 'contact-form-7' ) ); ?></h2>

<?php
	$tag_generator = webflower_TagGenerator::get_instance();
	$tag_generator->print_buttons();
?>

<textarea id="webflower-form" name="webflower-form" cols="100" rows="24" class="large-text code" data-config-field="form.body"><?php echo esc_textarea( $post->prop( 'form' ) ); ?></textarea>
<?php
}

function webflower_editor_panel_mail( $post ) {
	webflower_editor_box_mail( $post );

	echo '<br class="clear" />';

	webflower_editor_box_mail( $post, array(
		'id' => 'webflower-mail-2',
		'name' => 'mail_2',
...
function webflower_editor_box_mail( $post, $args = '' ) {
...
function webflower_editor_panel_messages( $post ) {
	$messages = webflower_messages();

	if ( isset( $messages['captcha_not_match'] )
	&& ! webflower_use_really_simple_captcha() ) {
...
<?php
	...
	foreach ( $messages as $key => $arr ) {
		$field_id = sprintf( 'webflower-message-%s', strtr( $key, '_', '-' ) );
		$field_name = sprintf( 'webflower-messages[%s]', $key );
...
function webflower_editor_panel_additional_settings( $post ) {
	$desc_link = webflower_link(
...
<textarea id="webflower-additional-settings" name="webflower-additional-settings" cols="100" rows="8" class="large-text" data-config-field="additional_settings.body"><?php echo esc_textarea( $post->prop( 'additional_settings' ) ); ?></textarea>
</fieldset>
<?php
}
